...
static void splitLevinson_anti( const double* correlation, 
								double  *reflect, double *alphap, 
								double* alphaq, int amountLsfs );

static void calculateSymmetricLsfs(double *alphap, double *teta, int amountLsfs);

static void calculateAntiSymmetricLsfs(double *alphaq, double *teta, int amountLsfs);

static void Tqli ( double *d, double  *e, int order);
...
int 
correlation2Lsfs( const double *correlation, double *lsfs, double* reflect, int amountLsfs)
{

    double *alphap, *alphaq;
    int i;

	alphap = (double*)malloc(sizeof(double)*(amountLsfs+1));
	alphaq = (double*)malloc(sizeof(double)*(amountLsfs+1));
	...
	qsort(lsfs, amountLsfs, sizeof(double), compare);
	free(alphaq);
	free(alphap);
	return(1);
}
...
void 
splitLevinson_anti( const double* correlation, double  *reflect, 
					double *alphap, double* alphaq, int amountLsfs )
{

    int   i, j, t, l;

    double *taup, *lambda, *p_lin;

	/* allocate dynamic RAM */
	taup 	= (double*) malloc(sizeof(double)*(amountLsfs+1));
	lambda 	= (double*) malloc(sizeof(double)*(amountLsfs+1));
	p_lin 	= (double*) malloc(sizeof(double)*(amountLsfs+2)*(amountLsfs+1));
...
void 
calculateSymmetricLsfs(double *alphap, double *teta, int amountLsfs)
{
   int i, j;
   double *d, *e;

   /* allocate dynamic RAM */
   d = (double*) malloc(sizeof(double)*((amountLsfs/2)+1));            /* d[] holds main diagonal */
   e = (double*) malloc(sizeof(double)*((amountLsfs/2)+1));            /* e[] holds sub diagonal */

   /*   length m_2+1 is due to Tqli shift */
   
   alphap[1]=2.0*alphap[1];   /* start value is doubled, ref.[1]-(29)
...
void 
calculateAntiSymmetricLsfs(double *alphaq, double *teta, int amountLsfs)
{
	int i, j;
	double *d, *e;

   /* allocate dynamic RAM */
   d = (double*) malloc(sizeof(double)*((amountLsfs/2)+1));            
   /* d[] holds main diagonal */

   e = (double*) malloc(sizeof(double)*((amountLsfs/2)+1));            
...
void 
Tqli ( double *d, double  *e, int order)
{
    int     m, l, i, order_2;
